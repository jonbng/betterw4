...
/**
 * W4 session cookies. The only primary cookie is `PHPSESSID`.
 *
 * [autologinkey] is unused on W4 (Lectio leftover kept for JSON compatibility).
 */
data class W4Credentials(
    ...
    /** Must not contain the primary cookie name. */
    val additionalCookies: Map<String, String> = emptyMap(),
) {
    val isValid: Boolean
        get() = sessionId.isNotEmpty() && sessionIdExpiresAt.isAfter(Instant.now())

    fun clearingSessionId(): W4Credentials {
    ...
    fun withSessionId(newSessionId: String): W4Credentials {
        val extras = additionalCookies.toMutableMap().apply {
            remove(COOKIE_SESSION_ID)
        }
        return copy(
    ...
    fun seededIsLoggedIn(): W4Credentials {
    ...
    companion object {
        const val COOKIE_SESSION_ID = "PHPSESSID"
        const val COOKIE_AUTOLOGIN = "autologinkeyV2"
        const val COOKIE_IS_LOGGED_IN = "isloggedin3"

        val PRIMARY_COOKIE_NAMES = setOf(COOKIE_SESSION_ID)

        fun fromJson(json: String): W4Credentials? = try {
