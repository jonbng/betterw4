...
/**
 * Authenticated W4 download + FileProvider open/save/share.
 */
@Singleton
class AttachmentRepository @Inject constructor(
    ...
    private suspend fun prepareFile(ref: AttachmentRef): AppResult<CachedAttachment> {
        ...
        return when (val result = w4.get(absolute, priority = FetchPriority.Important)) {
    ...
    private fun materialize(
        ...
        if (looksLikeHtmlFailure(bytes, contentType, response.body)) {
            return if (
                W4Html.isLoginHtml(response.body) ||
                W4Html.isAjaxLoginRequired(response.body)
            ) {
    ...
    private fun looksLikeHtmlFailure(
        ...
        if (AttachmentMime.sniff(bytes) == null &&
            (sample.contains("<form") || W4Html.isLoginHtml(body))
        ) {
            return true
        }
        return false
    ...
    private fun saveToDownloads(cached: CachedAttachment) {
        ...
        val values = ContentValues().apply {
            ...
            put(MediaStore.Downloads.RELATIVE_PATH, Environment.DIRECTORY_DOWNLOADS + "/BetterW4")
        }
