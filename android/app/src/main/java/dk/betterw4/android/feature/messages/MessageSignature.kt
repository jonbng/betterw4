...
/**
 * BetterW4 signature injected at send/reply time (extension parity).
 *
 * BBCode so Lectio renders a download link.
 */
object MessageSignature {
    const val BBCODE =
        "\n\n[url=https://w4.jonathanb.dk/download]Sendt med BetterW4[/url]"
    ...
    fun appendIfNeeded(
        ...
        if (body.contains("Sendt med BetterW4", ignoreCase = true)) return body
        return body + BBCODE
    }
}
