...
sealed class HtmlSegment {
    data class Text(val html: String) : HtmlSegment()
    data class Image(val url: String, val alt: String) : HtmlSegment()
    data object Divider : HtmlSegment()
}
...
object HtmlSegments {
     ...
     */
    fun parse(html: String?): List<HtmlSegment> {
    ...
    fun extractImageUrls(html: String?): List<String> =
        parse(html).mapNotNull { (it as? HtmlSegment.Image)?.url }.distinct()
    ...
    private fun mergeAdjacentText(segments: List<HtmlSegment>): List<HtmlSegment> {
        if (segments.isEmpty()) return segments
        val out = mutableListOf<HtmlSegment>()
        val buf = StringBuilder()
        fun flush() {
            if (buf.isEmpty()) return
            out += HtmlSegment.Text(buf.toString())
            buf.clear()
        }
        for (seg in segments) {
            when (seg) {
