    ...
    Scaffold(
        ...
        Column(
            ...
            if (!state.needsOtp) {
                ...
                Text(
                    text = when (state.lastSchool?.reason) {
            ...
            Button(
                onClick = {
                    hideKeyboard()
                    if (state.needsOtp) viewModel.submitOtp() else viewModel.submitPassword()
                },
                enabled = if (state.needsOtp) state.canSubmitOtp else state.canSubmitPassword,
                modifier = Modifier.fillMaxWidth(),
            ) {
                if (state.loggingIn) {
                ...
                Text(
                    stringResource(
                        if (state.needsOtp) R.string.login_otp_submit else R.string.login_submit,
                    ),
                )
            }
