...
interface CredentialStore {
    fun saveCredentials(credentials: W4Credentials, studentId: String)
    fun loadCredentials(studentId: String): W4Credentials?
    fun updateCredentials(credentials: W4Credentials, studentId: String)
...
class EncryptedCredentialStore(
    ...
    override fun saveCredentials(credentials: W4Credentials, studentId: String) {
        prefs.edit {
            putString(credentialsKey(studentId), credentials.seededIsLoggedIn().toJson())
        }
    }

    override fun loadCredentials(studentId: String): W4Credentials? {
        val raw = prefs.getString(credentialsKey(studentId), null) ?: return null
        return W4Credentials.fromJson(raw)
    }

    override fun updateCredentials(credentials: W4Credentials, studentId: String) {
...
class InMemoryCredentialStore : CredentialStore {
    private val creds = mutableMapOf<String, W4Credentials>()
    ...
    override fun loadCredentials(studentId: String): W4Credentials? = creds[studentId]
