 ...
 * - Danish 7-step **or** IB 1–7 grades count toward averages (never mixed).
 * - Display uses Danish decimal comma.
 */
object GradeAverage {

    private val IB_PREFERRED_COLUMN_ORDER = listOf(
        "final",
        "awarded",
        "predicted",
        "term-2",
    ...
    fun defaultColumnKey(columns: List<GradeColumn>, rows: List<GradeRow>): String? {
        ...
        for (key in IB_PREFERRED_COLUMN_ORDER) {
            if (columns.any { it.key == key } && rows.any { it.cell(key) != null }) {
                return key
            }
        }
    ...
    /**
     * True when the report looks like IB 1–7 rather than Danish 7-trin.
     */
    fun looksLikeIb(columns: List<GradeColumn>, rows: List<GradeRow>): Boolean {
        val keys = columns.map { it.key.lowercase() }
        if (keys.any { it.contains("standpunkt") || it.contains("årskarakter") || it.contains("eksamens") }) {
            return false
        }
        val nums = rows.flatMap { it.grades.values }.mapNotNull { gradeToNumber(it.value) }
        if (nums.isEmpty()) return columns.isNotEmpty()
        return nums.all { it in 1.0..7.0 } && nums.none { it > 7.0 }
    }

    /**
     * Weighted average for a single column. Returns null when no numeric scale values.
     */
    ...
    /**
     * Map a grade string to a numeric value. Danish 7-step first, then IB 1–7.
    ...
    fun gradeToNumber(raw: String): Double? {
        ...
        // Accept "7,0" / "7.0" only when they map to a known step or IB 1–7
        ...
        return when (asIntLike) {
            ...
            "0" -> 0.0
                ...
                ?: asIntLike.toIntOrNull()?.takeIf { it in 1..7 }?.toDouble()
        }
    }
    ...
    /**
     * Progress 0…1 on Danish 7-trin (−3…12) or IB 1–7. Null if not a scale grade.
     */
    fun progressForGrade(value: String?, ibScale: Boolean = false): Float? {
        val n = value?.let { gradeToNumber(it) } ?: return null
        if (ibScale) {
            return ((n - 1.0) / 6.0).toFloat().coerceIn(0f, 1f)
        }
