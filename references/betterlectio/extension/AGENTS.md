...
- `lib/elevfeedback-frame.ts` - Chrome-strip CSS/helpers for the editor iframe (master menu, `#s_m_HeaderContent_subnav_div`, entity nav, tabs, TOC). Relocates **Nyt** out of the hidden TOC. Used by `elevfeedback-frame.content.ts` at `document_start`.
