...
struct SubjectSettingsView: View {
    ...
    var body: some View {
        List {
            Section {
                ...
                Text("Tryk på et fag for at ændre dets navn og farve.")
            }
            ...
            Section {
                Button(role: .destructive) {
                    ...
                    Label("Nulstil alle tilpasninger", systemImage: "arrow.counterclockwise")
                }
                .disabled(!hasAnyCustomizations)
            } footer: {
                Text("Dette vil fjerne alle dine tilpassede navne og farver.")
            }
        }
        .navigationTitle("Fagindstillinger")
        .sheet(item: $selectedSubject) { subject in
            SubjectEditSheet(
                subject: subject,
                currentColor: currentColor(for: subject),
                currentName: settingsStore.customName(for: subject.code) ?? "",
                defaultName: settingsStore.defaultName(for: subject.code) ?? SubjectMapper.defaultName(for: subject.code, fallback: subject.name),
        ...
        .alert("Nulstil alle tilpasninger?", isPresented: $showResetConfirmation) {
            Button("Annuller", role: .cancel) {}
            Button("Nulstil", role: .destructive) {
                settingsStore.resetAllLessonMappings()
            }
        } message: {
            Text("Er du sikker på, at du vil fjerne alle tilpassede navne og farver?")
        }
        .task {
    ...
    private func currentDisplayName(for subject: SubjectMapper.SubjectInfo) -> String {
        settingsStore.displayName(for: subject.code)
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                // Custom color picker
                Section("Brugerdefineret farve") {
                    ColorPicker("Vælg farve", selection: $selectedColor)
                }
                ...
                Section {
                    Button(role: .destructive) {
                        ...
                        Label("Gendan standardindstillinger", systemImage: "arrow.counterclockwise")
                    }
                }
            }
