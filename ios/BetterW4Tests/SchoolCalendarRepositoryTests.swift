...
final class SchoolCalendarRepositoryTests: XCTestCase {
    ...
    func testICSIsLoadedOnceAcrossWeeksUntilForced() async throws {
        ...
        let countAfterSameWeek = await counter.count
        XCTAssertEqual(countAfterSameWeek, 1, "the same week must reuse the in-memory ICS")
        XCTAssertEqual(first?.allEvents.map(\.id), second?.allEvents.map(\.id))

        _ = await repository.weekOverlay(year: 2026, week: 34)
        let countAfterNeighbour = await counter.count
        XCTAssertEqual(countAfterNeighbour, 1, "a neighbouring week must reuse the same ICS body")

        _ = await repository.weekOverlay(year: 2026, week: 33, forceRefresh: true)
        let countAfterRefresh = await counter.count
        XCTAssertEqual(countAfterRefresh, 2, "forceRefresh must re-read the feed")
    }
