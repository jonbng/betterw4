...
enum W4AbsenceParser {
    ...
    nonisolated static func parseList(_ html: String, source: AttendanceSource) -> AttendanceList {
        ...
        let columns = header.map { columnMap(fromHeader: cellElements(of: $0)) } ?? Columns()
        if columns.isEmpty {
            warn("list (\(source.rawValue)): no usable header row")
            return .empty(source: source, meter: meter, message: pageNote(in: scope))
        }
