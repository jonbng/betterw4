...
/// One role's `Section`, lifted out of `OnDutyView.body`.
///
/// It was inline, and it did not compile: a `Section` with a `header:` that itself contains a
/// `ForEach`, nested inside another `ForEach`, inside an `if`/`else` in a `List`, defeated the
/// type checker. It gave up on SwiftUI's `ForEach` entirely and reported the failure against
/// Charts' `ChartContentBuilder` overload — an error that names a framework this file does not
/// import and a line that is not the problem. Giving the section a concrete type is what makes
/// the inference tractable again.
private struct OnDutyRoleSection: View {
    let group: OnDutyGroup
