...
struct ScheduleView: View {
    ...
    /// The line above the day strip: which month the visible week sits in, a way back to
    /// today when the student has paged away, and the ISO week number W4 itself numbers by.
    ///
    /// This used to be a floating capsule straddling the top edge of the strip card, which the
    /// navigation bar and the "now / next lesson" header sat on top of. It is an ordinary row
    /// in the layout now, so nothing can cover it.
    private var weekContextBar: some View {
        HStack(spacing: 8) {
            Text(weekRangeTitle)
    ...
    /// `August 2026`, or `Aug – Sep 2026` for a week that straddles two months.
    private var weekRangeTitle: String {
        let start = W4Dates.startOfWeek(containing: selectedDate)
        let end = W4Dates.adding(days: 6, to: start)
        let startMonth = W4Dates.calendar.component(.month, from: start)
        let endMonth = W4Dates.calendar.component(.month, from: end)
        let startYear = W4Dates.calendar.component(.year, from: start)
        let endYear = W4Dates.calendar.component(.year, from: end)

        if startMonth == endMonth, startYear == endYear {
            return "\(W4Dates.monthName(of: start)) \(startYear)"
        }
        if startYear == endYear {
            return "\(W4Dates.shortMonthName(of: start)) – \(W4Dates.shortMonthName(of: end)) \(endYear)"
        }
        return "\(W4Dates.shortMonthName(of: start)) \(startYear) – \(W4Dates.shortMonthName(of: end)) \(endYear)"
    }

    private var isViewingToday: Bool {
        W4Dates.isSameDay(selectedDate, currentTime)
    }

    private var isViewingCurrentWeek: Bool {
        W4Dates.isSameDay(
            W4Dates.startOfWeek(containing: selectedDate),
            W4Dates.startOfWeek(containing: currentTime)
        )
    }
    ...
    /// The ISO week number of the visible week, tinted when that week is the current one.
    private var weekNumberBadge: some View {
        let week = W4Dates.isoWeek(of: selectedDate).week
        return Text("Week \(week)")
            .font(.caption.weight(.semibold))
            .monospacedDigit()
            .foregroundStyle(isViewingCurrentWeek ? Color.accentColor : Color.secondary)
            .padding(.horizontal, 10)
            .padding(.vertical, 4)
            .background(
                Capsule()
            ...
            .accessibilityLabel(
                isViewingCurrentWeek ? "Week \(week), the current week" : "Week \(week)"
            )
    }
}
