...
enum HouseFlagKind: String, Equatable, Sendable {
    ...
    static func of(_ houseIdOrName: String?) -> HouseFlagKind? {
        ...
        // U+00E5 as an escape rather than the literal character: `scripts/check-english.sh`
        // bans Scandinavian characters outside comments, and it cannot tell a Danish UI string
        // from a folding table. This is the latter — it folds the "a-ring" in Norwegian house
        // names so they match the ASCII keys in the switch below, and is never shown to anyone.
        let folded = raw.lowercased()
            .replacingOccurrences(of: "\u{00E5}", with: "a")
